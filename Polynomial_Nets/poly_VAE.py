...
class VAE_Cheby_L3(nn.Module):
    def __init__(self, image_size=32*32, h_dim=100, z_dim=4, rank=10):
        super(VAE_Cheby_L3, self).__init__()
        self.encoder = Chebyshev_L3(image_size, rank, h_dim)   
        self.fc1 = nn.Linear(h_dim, z_dim)
        self.fc2 = nn.Linear(h_dim, z_dim)
        self.fc3 = nn.Linear(z_dim, h_dim)
        
        self.decoder = nn.Sequential(Chebyshev_L3(h_dim, rank, image_size), nn.Sigmoid())
...
class VAE_Cheby_L3_sparse(nn.Module):
    def __init__(self, image_size=32*32, h_dim=100, z_dim=4, rank=10):
        super(VAE_Cheby_L3_sparse, self).__init__()
        self.encoder = Chebyshev_L3_sparse(image_size, rank, h_dim)   
        self.fc1 = nn.Linear(h_dim, z_dim)
        self.fc2 = nn.Linear(h_dim, z_dim)
        self.fc3 = nn.Linear(z_dim, h_dim)
        
        self.decoder = nn.Sequential(Chebyshev_L3_sparse(h_dim, rank, image_size), nn.Sigmoid())
